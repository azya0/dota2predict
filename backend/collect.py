...
async def collect_pro_matches():
    ...
    for counting, (id, _) in enumerate(parse_pro_matches(await get_pro_matches())):
        print(f"Collecting pro matches id {counting}...")
    ...
    while matches_id:
        ...
        for index, match_data in enumerate(matches_data):
            if not isinstance(match_data, Exception):
                final_corutines.append(save_match(parse_match(match_data, is_pro_match=True)))
                continue
        ...
        print(f"Limited: {len(matches_id)}")
...
if __name__ == "__main__":
    print("Initialization new event loop")
